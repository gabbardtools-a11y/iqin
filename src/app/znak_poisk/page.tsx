...
const DATABASES = [
  {
    ...
    url: "https://naytea.ru/",
    category: "(РФ + INT)",
    description: "Онлайн-проверка товарного знака по российским и международным базам. Один запрос — результаты из реестров Роспатента, ВОИС, EUIPO. Наша собственная разработка.",
    recommended: true,
  },
  {
    name: "LINKMARK",
  ...
  return (
    <SearchDatabasesPage
      ...
      intro={[
        ...
        "Ниже — 12 проверенных сервисов для поиска. Рекомендуем начать с NayTea.ru — это наш собственный сервис, который объединяет ИИ-поиском сразу несколько баз (Роспатент, ВОИС, EUIPO) в одном запросе. Затем можно проверить результаты в каждой отдельной базе — LINKMARK, WIPO, USPTO. Для определения нужных классов МКТУ используйте справочники MKTU.INFO или мкту.рус.",
      ]}
      databases={DATABASES}
      accent="#60A5FA"
    />
